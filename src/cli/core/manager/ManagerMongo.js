...
class Manager {
        ...
        .then(
          templatesList => {
            return cmsTemplates.template
              ...
              .then(
                templatesText => {
                  return cmsTemplates.template //id
  ...
  async updatePostInList(revisionPath) {
    ...
    const json = await cmsData.file.get(revisionPath);
    console.log('update post in list', revisionPath)
    let index
    let merged = {}
    let revision = await cmsData.file.get(revisionPath)
    ...
    if (found.length > 0) {
      ...
      let rev = []
      rev.push(revision)
      console.log('rev revision', revision)
   ...
   */
  async updateList() {
    this._list = await cmsData.file.getAllWithKeys(this._whereKeys)
